...
static void *buddy_alloc_impl(IAllocator *self, size_t size) {
  ...
  if (target_order < ctx->min_order)
    target_order = ctx->min_order;
  if (target_order > ctx->max_order)
    return NULL;

  int current_order = target_order;
  while (current_order <= ctx->max_order &&
         ctx->free_lists[current_order] == NULL) {
    current_order++;
  }

  if (current_order > ctx->max_order)
    return NULL;
...
IAllocator create_buddy_alloc(void *buffer, size_t size) {
  ...
  ctx->max_order = 0;
  while ((1ULL << (ctx->max_order + 1)) <= size)
    ctx->max_order++;
